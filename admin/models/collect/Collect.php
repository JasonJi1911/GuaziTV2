...
//        return $this->hasOne(IpAddress::className(), ['id' => 'city_id']);
    ...
    public function vod_xml($param,$html='')
    {
        ...
        if(empty($xml)){
            ...
            if(empty($xml)) {
                return ['code' => 1002, 'msg' => 'XML格式不正确，不支持采集', 'url'=> $url];
            }
        }
...
//        $videoChapter = new VideoChapter();
        ...
        foreach ($cj_play_url_arr as $v2)
        {
            ...
            foreach ($urlList as $url)
            {
                ...
                if(is_numeric(trim($title)))
                    $epiIndex = intval(trim($title));
                else
                    $epiIndex = 1;
    ...
    private function ParseVideoChapter($video_id, $play_from, $play_url, $source_id, $channel_id, $video_name, $filterList = '')
    {
        ...
        $titleBak = ['正片', 'HD', '高清', 'HD1080P中字', 'HD高清', 'DVD', 'BD超清中字', 'BD国语超清', '高清国语中文字幕', 'BD', 'BD高清中字', 'HD720P中字', 'BD高清', 'HD国语', '1080P', 'BD720p中英字幕', '4k', '1080', '高清(原声版)', '蓝光', '备用', $video_name ];
//        $videoChapter = new VideoChapter();
